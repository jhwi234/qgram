...
def calculate_redundancy(H, H_max):
    ...
    :param H: Actual entropy of the language (bits per character).
    :param H_max: Maximum possible entropy given the alphabet (bits per character).
    :return: Redundancy percentage of the language.
    """
...
def process_corpora(corpus):
    for corpus_name in corpus:
        ...
        klm_file_path = train_kenlm_model(text_for_kenlm, 6, model_name)
...
process_corpora(['brown', 'gutenberg', 'reuters'])
