...
def plot_normalized_stacked_histogram(ax, dataset, base_color, label, column_name, threshold=0.75, bins=30):
    ...
    ax.bar(bins[:-1], valid_proportions, width=np.diff(bins), align='edge', color=base_color, alpha=0.75, label=f'Valid' if column_name == "Top1_Is_Valid" else 'Accurate', edgecolor='black')
    ax.bar(bins[:-1], invalid_proportions, width=np.diff(bins), align='edge', color='gray', alpha=0.65, label=f'Invalid' if column_name == "Top1_Is_Valid" else 'Inaccurate', bottom=valid_proportions, edgecolor='black')
    ...
    if valid_proportions[first_threshold_bin_index] >= threshold:
        ...
        ax.annotate(f'{first_threshold_bin:.2f}', xy=(first_threshold_bin, 0.9), xytext=(first_threshold_bin - 0.15, 0.75),
                    arrowprops=dict(facecolor='black', shrink=0.05), fontsize=16, color='black', fontweight='bold', ha='right')
    ...
    ax.set_title(f'{label} Dataset ({column_name.split("_")[-1].capitalize()})')
...
def plot_and_save_figures(column_name, filename):
    ...
    for (label, dataset), color, ax in zip(loaded_datasets.items(), colors, axs.flatten()):
        plot_normalized_stacked_histogram(ax, dataset, color, label, column_name)
    ...
    if combined_data:
        ...
        plot_normalized_stacked_histogram(ax, combined_dataset, 'tab:blue', 'Combined', column_name)
        fig.savefig(output_dir / f'combined_{filename}.png')
        plt.close(fig)

# Create plots for "Top1_Is_Accurate"
plot_and_save_figures("Top1_Is_Accurate", 'normalized_accurate_stacked_histograms')

# Create plots for "Top1_Is_Valid"
plot_and_save_figures("Top1_Is_Valid", 'normalized_valid_stacked_histograms')
